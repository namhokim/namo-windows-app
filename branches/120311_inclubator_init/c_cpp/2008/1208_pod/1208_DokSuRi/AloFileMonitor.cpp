...
bool CompareFilesize(FILE_SIZE before, FILE_SIZE after)
{
	if(before.fileSizeHigh == after.fileSizeHigh && before.fileSizeLow == after.fileSizeLow) return true;
	else return false;
}

FILE_SIZE GetFilesize(_STRING filePath)
{
	FILE_SIZE fs;
	HANDLE hFile;
	...
	if(hFile != INVALID_HANDLE_VALUE) {
		fs.fileSizeLow = GetFileSize(hFile, &(fs.fileSizeLow));
		CloseHandle(hFile);
	}
	else fs.fileSizeLow = fs.fileSizeHigh = -1;
	return fs;
}
...
DWORD WINAPI thFnMonitor(LPVOID lpParam)
{
	...
	if( dwChangeHandles[1] == NULL) return 0;
	...
	{ 
		...
		dwWaitStatus = WaitForMultipleObjects(
			2,					// nCount : The number of object handles
			dwChangeHandles,	// *lpHandles : An array of object handles
			FALSE,				// bWaitAll : all or any one signaled
			INFINITE);			// dwMilliseconds : The time-out interval
		...
		case WAIT_OBJECT_0: 
			...
			{
				FILE_SIZE fs1 = GetFilesize(pMonitor->m_filePath);
				::Sleep(500);
				FILE_SIZE fs2 = GetFilesize(pMonitor->m_filePath);
				std::cout << fs1.fileSizeLow << " : " << fs2.fileSizeLow << std::endl;
				if(CompareFilesize(fs1, fs2) == true) {
			...
			if (FindNextChangeNotification(dwChangeHandles[0]) == FALSE) return 0;
			break;
		case WAIT_OBJECT_0 + 1:		// WAIT_OBJECT_0 + nCount(2) ? 1
			return 0;
        default: 
            return 0;
		}
...
{
	if(CAloFileMonitor::m_filePath.empty() ||
		CAloFileMonitor::m_dirPath.empty() ||
