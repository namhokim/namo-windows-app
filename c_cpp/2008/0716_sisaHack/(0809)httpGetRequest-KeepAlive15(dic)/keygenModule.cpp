...
//			printf("%s%c\n", stringBuffer, dictionary[i]);	// 출력
			...
			do {
				...
				printf("%s\t%d\t%.2fms\n", buffer, resultQuery, (GetHDTimer()-timer)*1024);
...
//			printf("%s%c\n", stringBuffer, dictionary[i]);	// 출력
			...
			if(remainRetry <= 0) return FALSE;
			if(0 == strcmp(finalData, buffer)) return FALSE;	// 마지막 검색하는 단어이면
		}
	}
	else {
		size_t location;
...
{
	char tmpBeginString[MAX_PASSWORD]={0};			// 초기화 추가(08.07.27)
	char buffer[MAX_LENGTH]={0};
	int level;
	SYSTEMTIME stCurrStartTime, stCurrEndTime;		// 시스템 시간
	...
	Display2(level,buffer,beginString,level);				// strcat_s에서 변경(08.07.27)
...
void controlEntry3(char * id, char * beginString, char * endString)
{
	...
	// 임시변수로 넘김
	strcpy_s(tmpBeginString, MAX_PASSWORD, beginString);	// strcat_s에서 변경(08.07.27)
	printf("from : %s\n", tmpBeginString);					// beginString에서 변경(08.07.27)
	printf("to   : %s\n", endString);
