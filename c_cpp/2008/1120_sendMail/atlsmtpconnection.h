...
// STMP AUTH
#define NAMOSMTP_AUTH_STEP "334"
#define NAMOSMTP_AUTH_SUCC "235"
#define NAMOSMTP_AUTH_ID_LENGTH			255
#define NAMOSMTP_AUTH_PASSWORD_LENGTH	255
...
private:
	...
	{
		CSMTPConnection::lastMessage = msg;
	}
...
public:
	...
	{
		return CSMTPConnection::lastMessage;
	}
	...
	// Attempt to connect to the socket with AUTHORIZATION
	// lpszHostName - the host name to connect to
	// lpszID - login ID
	// lpszPassword - login Password
	BOOL Connect(LPCTSTR lpszHostName, PCCH szID = NULL, PCCH szPassword = NULL, DWORD dwTimeout = 10000) throw()
	{
		...
		{
			CSMTPConnection::setLastMessage("Hostname find Error.");
			return FALSE;
		}
		...
		while ((pAI = address.GetAddrInfo(nIndex++)) != NULL)
		{
			...
			m_hSocket = INVALID_SOCKET;
			CSMTPConnection::setLastMessage("Soccket Connect Error.");
		}
		...
		if (bRet)
		{
			...
			{
				bRet = FALSE;
				CSMTPConnection::setLastMessage("Create Event Error.");
			}
		}
		...
		if (bRet)
		{
			...
			if (bRet)
			{
				if (strncmp(szBuf, ATLSMTP_CONN_SUCC, ATLSMTP_RETCODE_LEN))
				{
					bRet = FALSE;
					szBuf[nBufLen]='\0';
					CSMTPConnection::setLastMessage(szBuf);
				}
			}
		}
		...
		{
#if _SECURE_ATL
			nBufLen = sprintf_s(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "HELO localhost\r\n");
#else
			nBufLen = _snprintf(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "HELO localhost\r\n");
#endif
			if (nBufLen > 0)
			{
		...
		if(szID != NULL && szPassword != NULL) {
			// Send AUTH LOGIN command and get reply
			if (bRet)
			{
#if _SECURE_ATL
				nBufLen = sprintf_s(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "AUTH LOGIN\r\n");
#else
				nBufLen = _snprintf(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "AUTH LOGIN\r\n");
#endif
				if (nBufLen > 0)
				{
					bRet = AtlSmtpSendAndCheck((HANDLE)m_hSocket, szBuf, nBufLen, szBuf, &nBufLen, 
						ATLSMTP_MAX_LINE_LENGTH, NAMOSMTP_AUTH_STEP, &m_Overlapped);
				}
				else
				{
					bRet = FALSE;
			...
			// Send login ID with base 64 encoding(+"=") and get reply
			if (bRet)
			{
				char szLoginID[NAMOSMTP_AUTH_ID_LENGTH];
				int lengthEncoding = NAMOSMTP_AUTH_ID_LENGTH;
				bRet = Base64Encode((BYTE *)szID,(int)strlen(szID),
					szLoginID,&lengthEncoding, ATL_BASE64_FLAG_NOPAD);
				szLoginID[lengthEncoding] = '\0';
#if _SECURE_ATL
				nBufLen = sprintf_s(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "%s=\r\n", szLoginID);
#else
				nBufLen = _snprintf(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "%s=\r\n", szLoginID);
#endif
				if (nBufLen > 0)
				{
					bRet = AtlSmtpSendAndCheck((HANDLE)m_hSocket, szBuf, nBufLen, szBuf, &nBufLen, 
			...
			// Send login Password with base 64 encoding(+"=") and get reply
			if (bRet)
			{
				char szLoginPassword[NAMOSMTP_AUTH_PASSWORD_LENGTH];
				int lengthEncoding = NAMOSMTP_AUTH_ID_LENGTH;
				bRet = Base64Encode((BYTE *)szPassword,(int)strlen(szPassword),
					szLoginPassword,&lengthEncoding, ATL_BASE64_FLAG_NOPAD);
				szLoginPassword[lengthEncoding] = '\0';
#if _SECURE_ATL
				nBufLen = sprintf_s(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "%s=\r\n", szLoginPassword);
#else
				nBufLen = _snprintf(szBuf, ATLSMTP_MAX_LINE_LENGTH+1, "%s=\r\n", szLoginPassword);
				...
				{
					bRet = AtlSmtpSendAndCheck((HANDLE)m_hSocket, szBuf, nBufLen, szBuf, &nBufLen, 
						ATLSMTP_MAX_LINE_LENGTH, NAMOSMTP_AUTH_SUCC, &m_Overlapped);
				}
				else
				{
					bRet = FALSE;
		...
		else CSMTPConnection::setLastMessage("Connect Success.");
	...
	inline BOOL Disconnect() throw()
	{
		if (!Connected())
		{
			CSMTPConnection::setLastMessage("Not Coneccted.");
			return FALSE;
		}
		...
		{
			CSMTPConnection::setLastMessage("Socket Shutdown Error.");
			return FALSE;
		}
		...
		CSMTPConnection::setLastMessage("Disconnect Success.");
		return TRUE;
	}
	...
	inline BOOL SendMessage(CMimeMessage& msg, LPCTSTR lpszRecipients = NULL, LPCTSTR lpszSender = NULL) throw()
	{
		...
		CSMTPConnection::setLastMessage("Mail sent successfully");
		return bRet;
	}
