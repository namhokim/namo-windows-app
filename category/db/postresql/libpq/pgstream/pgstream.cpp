...
int write_bytea(char* buf, int size)
{
  ...
  const int param_length[]={size};
  const int param_format[]={1};
  PGresult* res = PQexecParams(pgconn, "UPDATE tblob SET contents=$1", 
			       1, in_oid, params, param_length,
			       param_format, 1);
...
int read_bytea(char** buf, int* size)
{
  PGresult* res = PQexecParams(pgconn, "SELECT contents FROM tblob", 0, NULL,NULL,NULL,NULL,1);
  if (res && PQresultStatus(res)==PGRES_TUPLES_OK) {
    int f=PQfformat(res,0);
    *size=PQgetlength(res, 0, 0);
    printf("res format=%d,size=%d\n", f, *size);
    *buf=(char*)malloc(*size);
    memcpy(*buf, PQgetvalue(res, 0, 0), *size);
    Oid o=PQftype(res, 0);
    printf("oid=%d\n", o);
  }
  else if (res) {
...
#ifdef STANDALONE
void test_bytea(pg_cnx& cnx)
{
  try {
    pg_stmt("DROP TABLE btest", cnx);
  }
  catch (pg_excpt p) {
    if (p.errcode()!="42P01")	// let's ignore "table does not exist" error
      throw p;
  }
  pg_stmt("CREATE TABLE btest(k int, b bytea)", cnx);

  {
    pg_stream s("INSERT INTO btest VALUES(1, :bindata)", cnx);
    pg_bytea b;
    FILE* f = fopen("/home/daniel/src/manitou/xpm/pattern-about.png", "rb");
    fseek(f,0,SEEK_END);
    long sz=ftell(f);
    fseek(f,0,SEEK_SET);
    char buf[sz];
    sz=fread(buf, 1, sz, f);
    b.set_src_data(buf, (unsigned int)sz);
    fclose(f);
    s << b;
  }
  {
    pg_stream s1("SELECT b FROM btest WHERE k=1", cnx);
    pg_bytea b1;
    s1 >> b1;
    FILE *f = fopen("binfile", "w");
    fwrite(b1.get_data_ptr(), b1.get_data_size(), 1, f);
    fclose(f);

    pg_stream s2("UPDATE btest SET b=:p1<bytea> WHERE k=1", cnx);
    s2 << sql_null();
    //    pg_bytea b2;
    //    s2 << b2;
  }
...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    //cnx.connect("dbname=mail");
    cnx.set_option("bind_variables", false);
    cnx.set_option("nested_transactions", true);
    cnx.set_option("prepare_statements", true);
    test_bytea(cnx);
#if 0
    {
      pg_stream s("SELECT sm2 FROM test where sm=:p1", cnx);
      //    s.set_cursor(100);
      //      s << argv[1];
      s << "A";
      while (!s.eos()) {
